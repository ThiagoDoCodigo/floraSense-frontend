    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (email === 'admin@flora.com' && password === '123456') {
          resolve({ user: { id: '1', name: 'Admin', email }, token: 'mock-jwt-token' });
        } else {
          reject(new Error('Credenciais inválidas. Tente admin@flora.com / 123456'));
        }
      }, 1500);
    });
  }
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (email === 'admin@flora.com') {
          resolve({ message: 'Um link de recuperação foi enviado para o seu e-mail.' });
        } else {
